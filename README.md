# Spam-Elek
### Egy magyar nyelvű Discord bot, ami mindenféle vicces spam-eket küld!

Az üzenetetk küldése random időnként(10 perc-1 óra) történik, vagy ha valaki megjelöli a botot egy üzenetben ( @<bot_neve> ) \
A botnak megadott tokennek kell rendelkeznie üzenet írási és üzenet olvasási jogokkal is!
> Megjegyzés: Ez a bot hobbiból készült, és nem tervezem frissíteni a jövőben

Minták a "spam"-ekből:
- "Szerintem a kávé ma is a hősünk."
- "Valaki hallotta az éjfél harangját? Vagy csak a szomszédok dobolnak?"
- "Figyelem! Képzeletbeli ninja támadás!"
- "Random spam, de legalább barátságos spam!"

#### Ha új üzeneteket szeretnél hozzáadni, csak add hozzá a  "messages" listához!

# Használat
1. Klónozd ezt a repót, vagy töltsd le a **main.py** fáljt!
2. Töltsd le a **discord.py** könyvtárat ezzel a parnccsal: `pip install discord.py`
3. Írd bele a fáljba a **discord-bot tokened**, és a **channel ID**-t!
4. Indítsd el a fáljt!
